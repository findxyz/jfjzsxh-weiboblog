...
- **按日拆分**：范围会按日拆分，逐日以当天 00:00:00~23:59:59 调 searchProfile
  翻页。按日拆分避免按月/按年大范围翻页时分页边界丢数据（实测按月会丢）。
- 数据入同一张 `weibo_posts` 表，靠 mblogid 去重，与 mymblog 抓的数据混存。
- `--start`/`--end` 格式 `YYYY-MM-DD`，`--end` 含当天（23:59:59 +0800）。
- `--start` 晚于 `--end` 时自动交换（容错），正常使用 `--start` ≤ `--end`。
- 与 `--full`/`--all` 互斥，必须配合 `--uid`。
- 重跑同一范围不会重复入库（mblogid 去重）；某天抓取失败不会中断整体，会告警
  跳过当日继续下一天，已抓数据不丢。
